...
*/
        document.getElementById("encryptTextPad").value = "";document.getElementById("decryptTextPad").value = "";document.getElementById("teksti1").value = "";document.getElementById("teksti2").value = "";
        var encryptedFinalText = "";var decryptFinalText = "";
        const letters = [
        ...
        function crypt(){
            var maar = document.getElementById("count").value;
            encryptedFinalText = "";
            var rndm2 = Math.floor(Math.random() * 8) + 1; 
            var rndm3 = Math.floor(Math.random() * 8) + 1; 
            var enkryptattava = String(document.getElementById("teksti1").value);
            document.getElementById("encryptTextPad").value = "";
            ...
            document.getElementById("encryptTextPad").value = encryptedFinalText;
            document.getElementById("lengthEncry").innerHTML = "Encrypted length = " +  encryptedFinalText.length;
        }
        function decrypt(){
            var hkmpv = 2^53;
            decryptFinalText = "";
            document.getElementById("decryptTextPad").value = "";
            var dekryptattava = String(document.getElementById("teksti2").value);
            var currentChars = "";
            ...
            document.getElementById("decryptTextPad").value = decryptFinalText;
        }
        function copy(x){
            if(x == true){
                var copyText = document.getElementById("encryptTextPad");
                copyText.select();
                copyText.setSelectionRange(0, document.getElementById("encryptTextPad").value.length);
                navigator.clipboard.writeText(copyText.value);
            } else {
                var copyText = document.getElementById("decryptTextPad");
                copyText.select();
                copyText.setSelectionRange(0, document.getElementById("decryptTextPad").value.length);
                navigator.clipboard.writeText(copyText.value);
            }
        }
        function clearTXTfield(y){
            if(y == true){
        ...
        function move(x) {
            var elem11 = document.getElementById("myProgress1");
            ...
            if(x==true){
                ...
                var elem21 = document.getElementById("myProgress2");
                elem21.classList.remove("active");
                var qpt = 0;
                if (qpt == 0) {
                    qpt = 1;
